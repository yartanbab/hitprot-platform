...
* Check the `ConnectionStrings` in `appsettings.json` files under the `Apya.Platform.Web` and `Apya.Platform.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p 62c2cd20-9a93-47f2-b9f1-dfcc2d300e4f
```

> `62c2cd20-9a93-47f2-b9f1-dfcc2d300e4f` is the password of the certificate, you can change it to any password you want.
...
Run `Apya.Platform.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `Apya.Platform.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `Apya.Platform.Web`: ASP.NET Core MVC / Razor Pages application that is the essential web application of the solution.
