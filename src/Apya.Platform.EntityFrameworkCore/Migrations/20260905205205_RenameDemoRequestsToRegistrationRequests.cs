...
{
    /// <summary>
    /// Demo talebi → kayıt talebi dönüşümü.
    ///
    ...
    /// adaylarını — sessizce silerdi. Tablo yeniden adlandırılıyor, kaldırılan sütunlar
    /// düşürülüyor, yeni sütunlar varsayılanla ekleniyor.</para>
    ///
    /// <para>Eski satırlar kayıt talebi olarak HAYATTA KALIR ama kurum kimliği
    /// (unvan hariç: yetkili görevi, vergi no, adres) BOŞ gelir — demo formu bunları hiç
    /// sormuyordu. Panelde eksik görünmeleri beklenen davranıştır; host bu adaylarla
    /// iletişime geçip bilgileri tamamlar ya da kaydı kapatır.</para>
    ///
    /// <para>Durum sütunu OLDUĞU GİBİ kalır: <c>New(0)</c> ve <c>Closed(2)</c> yeni
    /// enum'da aynı sayıyı taşıyor, <c>Contacted(1)</c> ise yalnız ad değiştirdi
    /// (<c>InReview</c>). Bu yüzden durum için veri taşıma gerekmiyor.</para>
    /// </summary>
