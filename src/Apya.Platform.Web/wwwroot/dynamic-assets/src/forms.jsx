    ...
    <div className="text-slate-800">
      ...
      <div className="mb-5 flex flex-wrap items-center gap-2">
        <button
          ...
          className={`rounded-full px-3 py-1.5 text-xs font-semibold transition ${categoryFilter === '' ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
        >Tümü</button>
        {categories.map((c) => (
          <button
            key={c.id}
            onClick={() => setCategoryFilter(c.id)}
            className={`rounded-full px-3 py-1.5 text-xs font-semibold transition ${categoryFilter === c.id ? 'text-white' : 'text-slate-600 hover:opacity-80'}`}
            style={categoryFilter === c.id ? { backgroundColor: c.color || '#4f46e5' } : { backgroundColor: `${c.color || '#94a3b8'}20` }}
          >{c.icon ? `${c.icon} ` : ''}{c.name}</button>
        ))}
        {canManageCategories && (
          <button onClick={() => setShowCatModal(true)} className="ml-1 rounded-full border border-dashed border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-500 hover:border-slate-400 hover:text-slate-700">
            ⚙ Kategoriler
          </button>
        )}
      </div>
      ...
      {loading ? (
        <div className="py-16 text-center text-slate-400">Formlar yükleniyor…</div>
      ) : forms.length === 0 ? (
        <div className="rounded-2xl border-2 border-dashed border-slate-200 py-20 text-center">
          ...
          <h3 className="text-lg font-bold text-slate-700">{categoryFilter ? 'Bu kategoride form yok' : 'Henüz formun yok'}</h3>
          <p className="mt-1 text-sm text-slate-400">{categoryFilter ? 'Başka bir kategori seç veya yeni form oluştur.' : 'İlk formunu oluşturmak için başla.'}</p>
