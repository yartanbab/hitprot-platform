...
const fieldCls =
  'w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-800 placeholder-slate-400 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-100 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100';
...
function Field({ block, value, onChange }) {
  ...
  switch (block.type) {
        ...
        <div className="flex flex-col gap-2">
          {(s.options || []).map((o, i) => (
            <label key={i} className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-200">
              <input type="radio" name={block.id} checked={value === o} onChange={() => set(o)} className="h-4 w-4 text-indigo-600" />
              {o}
            </label>
          ))}
        </div>
    ...
    case BT.MultiSelect: {
      ...
      return (
        <div className="flex flex-col gap-2">
          {(s.options || []).map((o, i) => (
            <label key={i} className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-200">
              <input type="checkbox" checked={arr.includes(o)} onChange={() => toggle(o)} className="h-4 w-4 rounded text-indigo-600" />
              {o}
            </label>
          ))}
        </div>
    ...
    case BT.Rating:
      return (
        <div className="flex gap-1">
          {[1, 2, 3, 4, 5].map((n) => (
            <button type="button" key={n} onClick={() => set(n)} className={`text-3xl ${(value || 0) >= n ? 'text-amber-400' : 'text-slate-300'}`}>★</button>
          ))}
        </div>
      );
    case BT.Nps:
      return (
        <div className="flex flex-wrap gap-1.5">
          {Array.from({ length: 11 }, (_, i) => (
            <button type="button" key={i} onClick={() => set(i)} className={`h-9 w-9 rounded-lg border text-sm font-medium ${value === i ? 'border-indigo-600 bg-indigo-600 text-white' : 'border-slate-300 text-slate-600 dark:border-slate-600 dark:text-slate-300'}`}>{i}</button>
          ))}
        </div>
      );
    case BT.Address:
    ...
    case BT.FilePicker:
      return <input type="file" className="block w-full text-sm text-slate-500" onChange={(e) => set(e.target.files?.[0]?.name || '')} />;
    default:
...
function PublicForm({ slug }) {
  ...
  if (status === 'error') return <Centered><span className="text-rose-500">{errorMsg}</span></Centered>;
  if (status === 'done')
    return (
      <Centered>
        <div className="text-center">
          <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-emerald-100 text-3xl">✓</div>
          <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Teşekkürler!</h2>
          <p className="mt-2 text-slate-500">Yanıtınız başarıyla gönderildi.</p>
        </div>
      </Centered>
    );
  ...
  return (
    <div className="min-h-screen bg-slate-100 py-8 dark:bg-slate-900">
      {/* progress */}
      <div className="fixed inset-x-0 top-0 z-10 h-1.5 bg-slate-200 dark:bg-slate-700">
        <div className="h-full bg-indigo-600 transition-all duration-300" style={{ width: `${progress}%` }} />
      </div>

      <div className="mx-auto max-w-2xl px-4">
        <div className="overflow-hidden rounded-2xl bg-white shadow-sm dark:bg-slate-800">
          <div className="border-b border-slate-100 p-6 dark:border-slate-700" style={theme.primary ? { borderTopColor: theme.primary, borderTopWidth: 4 } : undefined}>
          ...
          <div className="flex flex-col gap-6 p-6">
            {fields.map((b) => {
              ...
              if (b.type === BT.SectionHeader)
                return <h2 key={b.id} className="border-b border-slate-200 pb-1 text-lg font-bold text-slate-700 dark:text-slate-200 dark:border-slate-700">{b.content}</h2>;
              if (b.type === BT.Paragraph)
                return <p key={b.id} className="text-sm text-slate-500">{b.content}</p>;
              return (
                <div key={b.id} className="flex flex-col gap-1.5">
                  ...
                  {s.helpText && <p className="text-xs text-slate-400">{s.helpText}</p>}
                  <Field block={b} value={answers[b.id]} onChange={onChange} />
                </div>
              );
            })}

            {errorMsg && <p className="text-sm font-medium text-rose-500">{errorMsg}</p>}
        ...
        <p className="mt-4 text-center text-xs text-slate-400">Apya Platform ile oluşturuldu</p>
      </div>
    </div>
  );
}
...
const Centered = ({ children }) => (
  <div className="flex min-h-screen items-center justify-center bg-slate-100 p-6 text-slate-500 dark:bg-slate-900">{children}</div>
);
