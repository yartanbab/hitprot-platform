...
**`EntityLinkAppService` — gözden geçirildi, kabul edilen düşük risk (2026-08-16):** `EntityLink` **IMultiTenant** (`CreationAuditedAggregateRoot`+`TenantId`) → ABP veri filtresi cross-tenant sızıntıyı önler. Kiracı içinde herhangi bir kimlikli kullanıcı polimorfik link (Task↔Document↔Project) kurabiliyor; **cross-module olduğu için doğru tek izin yok** → cerrahi gate mümkün değil, `[Authorize]` bırakıldı. Riski düşük (kiracı-izole metadata). **Olası derin takip:** link TÜKETİMİNDE (ör. görev detayında bağlı belge gösterilirken) hedef varlık izninin yeniden kontrol edilip edilmediği (IDOR açısı) — ayrı, daha derin inceleme.
