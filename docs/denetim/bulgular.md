...
#### TEST-002 🟠 `DOĞRULANDI` — SEC-001 sır boşaltması taze build'de tüm Web.Tests host'unu kırıyor
...
**Öneri:** Test-özel bir config ile dummy `ClientSecret` sağla — `test/Apya.Platform.Web.Tests/appsettings.Development.json` (veya `secrets` override) ile `OpenIddict:Applications:Platform_Web:ClientSecret` = dummy. Test gerçek OAuth yapmıyor; seed'in Confidential kontrolünü geçmesi için boş-olmayan değer yeterli. Değer prod sırrı DEĞİL, repo'ya girebilir (test-only). Bu SEC-001'in kapanışının parçası: sır boşaltıldı ama commit-dışı kaynak sağlanmadı → suite kırıldı.
