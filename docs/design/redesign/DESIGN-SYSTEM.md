...
- **P2 Buton/Chip:** `.btn/.form-control/.form-select` radius→`--apya-radius-lg`; `.btn-primary`→accent; `:focus-visible`→`--apya-shadow-focus`; **`.apya-chip` + `-positive/-negative/-warning/-brand/-ai/-neutral`** (color-mix %14 zemin + tonlu metin, dark varyantları dahil) — Razor ekranları `bg-{color}` badge'lerini P3'te buna geçirir.
...
- `.apya-overline` (uppercase KPI etiketi), `.kpi-icon-box`.
